    ...
    <q-card class="q-pa-md shadow-2" style="max-width: 600px; margin: auto;">
      ...
      <q-list separator>
        <q-item v-for="p in pengeluaran" :key="p.id">
          ...
          <q-item-section>
            <q-item-label>{{ p.keterangan }}</q-item-label>
            <q-item-label caption>
              Rp{{ Number(p.jumlah).toLocaleString('id-ID') }} - Tanggal: {{ p.tanggal }}
            </q-item-label>
          </q-item-section>

          <!-- Tampilkan tombol hanya jika user bukan peserta -->
      ...
      <!-- Tombol tambah hanya jika user bukan peserta -->
...
import { useUserStore } from '@/stores/userStore' // pastikan path ini sesuai

const pengeluaran = ref([])
const router = useRouter()
const $q = useQuasar()
const user = useUserStore().user // Ambil user dari store
